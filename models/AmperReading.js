...
amperReadingSchema.statics.getUserStats = async function(username) {
  ...
  return {
    ...
    percentage: stat.totalReadings > 0 ? Math.round((stat.highAmpCount / stat.totalReadings) * 100) : 0
  };
};
...
amperReadingSchema.statics.getRecentReadings = async function(username) {
  try {
    ...
    const allUserData = await this.find({ username: username }).select('createdAt amper').sort({ createdAt: -1 }).limit(5);
    console.log(`📊 All user data (last 5):`, allUserData.map(d => ({ createdAt: d.createdAt, amper: d.amper })));
